...
class GovernanceCliTests(unittest.TestCase):
    ...
    def test_baseline_requires_human_approval_and_release_zero(self) -> None:
        ...
        self.assertTrue(any(item["rule_id"] == "BAS007" for item in after_review_date))
    ...
    def test_adoption_contract_separates_tool_host_from_execution_target(self) -> None:
        ...
        self.assertTrue(any(item["rule_id"] == "VAL001" for item in diagnostics))
    ...
    def test_unsupported_production_code_language_is_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(any(item["rule_id"] == "SRCAN001" for item in diagnostics))

    def test_adoption_markdown_and_json_are_deterministic_and_stale_checked(
        self,
    ) -> None:
        manifest = valid_manifest()
        evidence = {
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            report.write_text(
                report.read_text(encoding="utf-8") + "manual\n", encoding="utf-8"
            )
            self.assertTrue(
                any(
                    item["rule_id"] == "DOC002"
            ...
            obsolete.write_text(
                json.dumps(
                    {
                        "_generated": "GENERATED BY govern-modular-event-architecture; DO NOT EDIT"
                    }
                ),
                encoding="utf-8",
            )
