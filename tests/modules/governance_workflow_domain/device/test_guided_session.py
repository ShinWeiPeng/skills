        ...
        "scenarios": [
            {
                ...
                "completion": {
                    "trigger_criteria": ["ready"],
                    "required_criteria": ["ready"],
                ...
                "guided_steps": [
                    {
                        "id": "power",
                        "instruction": "Power the target.",
                        "expected_observation": "Target powers on.",
                        "required": True,
                    },
                    {
                        "id": "capture",
                        "instruction": "Preserve the raw log.",
                        "expected_observation": "Raw log exists.",
                        "required": True,
                        "evidence_required": True,
                    },
                ],
                "criteria": [
                    {"id": "ready", "type": "event_sequence", "events": ["ready"]}
                ],
            }
        ],
    }
...
class GuidedContractTests(unittest.TestCase):
    def _ready(self, root: Path) -> tuple[dict, dict, dict]:
        ...
        review = finalize_response(
            session, response, lambda path: hash_evidence_reference(root, path, 4096)
        )
    ...
    def test_response_cannot_supply_verdict(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            review = finalize_response(
                session,
                response,
                lambda path: hash_evidence_reference(root, path, 4096),
            )
            self.assertEqual("BLOCKED", review["review_status"])
            self.assertTrue(
                any("verdict fields" in item for item in review["problems"])
            )
    ...
    def test_required_blocked_step_is_immediately_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual("BLOCKED", review["review_status"])
            self.assertTrue(
                any("required guided steps" in item for item in review["problems"])
            )
    ...
    def test_missing_unconfirmed_and_mismatched_identity_are_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertTrue(
                any("every session step" in item for item in review["problems"])
            )
    ...
    def test_revision_chain_and_tampered_review(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            second_review = finalize_response(
                session,
                second,
                lambda path: hash_evidence_reference(root, path, 4096),
                first,
    ...
    def test_blocked_revision_can_be_corrected_without_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            first_review = finalize_response(
                session, first, lambda path: hash_evidence_reference(root, path, 4096)
            )
            self.assertEqual("BLOCKED", first_review["review_status"])
            second = json.loads(json.dumps(first))
            second["revision"] = 2
            second["supersedes_sha256"] = canonical_sha256(first)
            second["steps"][0].update(
                {
                    "state": "completed",
                    "actual_observation": "power restored",
                    "blocked_reason": "",
                    "remediation": "",
                }
            )
    ...
    def test_gpt_and_offline_responses_have_same_fact_review(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            offline_review = finalize_response(
                session, offline, lambda path: hash_evidence_reference(root, path, 4096)
            )
...
class GuidedCliTests(unittest.TestCase):
    def test_alternate_output_syntax_cannot_bypass_admission(self):
        from unittest.mock import patch

        for spelling in ("--output=outside", "--out=outside"):
            with (
                self.subTest(spelling=spelling),
                patch("validate_on_device._execute") as execute,
    ...
    def test_direct_bundle_rejects_partial_identity_without_overwriting(self):
        from vod.evidence import write_bundle

        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            output = root / "artifacts/validation/run1"
            output.mkdir(parents=True)
            (output / ".run.json").write_text('{"run_id":"run1"}')
            (output / "result.json").write_text("original")
            value = guided_profile()
            value["_project_root"] = str(root)
            with self.assertRaises(ValueError):
                write_bundle(output, value, "smoke", root / "absent.log", {})
            self.assertEqual((output / "result.json").read_text(), "original")
    ...
    def test_blocked_step_stops_finalize_and_preserves_review(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            profile_path.write_text(
                yaml.safe_dump(guided_profile(), sort_keys=False), encoding="utf-8"
            )
            (project / "architecture").mkdir()
            (project / "architecture/manifest.yaml").write_text(
                json.dumps({"modules": [{"id": "test-device"}]})
            )
            artifacts = project / "artifacts/validation/guided"
            (validation / "layout.yaml").write_text(
                yaml.safe_dump(
                    {
                        "schema_version": 1,
                        "entries": [
                        ...
                        "output_bindings": [
                            {
                                "writer": "validate-on-device",
                                "scenario": "smoke",
                                "root": "artifacts/validation",
                                "target": {"module": "test-device"},
                            }
                        ],
                    }
                ),
            ...
            review = json.loads(
                (review_dir / "review.json").read_text(encoding="utf-8")
            )
            self.assertEqual("BLOCKED", review["review_status"])
    ...
    def test_prepare_finalize_evaluate_round_trip(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            session = json.loads(
                (artifacts / "session.json").read_text(encoding="utf-8")
            )
            response_path = artifacts / "response.template.json"
            response = json.loads(response_path.read_text(encoding="utf-8"))
            raw = project / "raw.log"
            raw.write_text(
                "\n".join(
                    [
                        f"VAL_SESSION_BEGIN run={session['run_id']} scenario=smoke observer=user t_ms=0 seq=1",
                        "VAL_EVENT name=ready t_ms=10 seq=2",
                        f"VAL_SESSION_END run={session['run_id']} reason=complete records=3 dropped=0 duration_ms=10 t_ms=10 seq=3",
                    ]
                ),
                encoding="utf-8",
            )
            ...
            (review_dir / "review.md").write_text(
                "manually edited display only", encoding="utf-8"
            )
            ...
            self.assertTrue(
                (evidence_dir / "guided" / "response.r1.json.sha256").exists()
            )
