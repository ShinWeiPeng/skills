...
from vod.evidence import evaluate, write_bundle as write_allocated_bundle
...
sys.path.insert(0, str(ROOT.parent / "verification-ladder/scripts"))


def write_bundle(output, value, *args, **kwargs):
    from run_storage import allocate_run

    root = output.parents[2]
    value = copy.deepcopy(value)
    value["_project_root"] = str(root.resolve())
    (root / "architecture").mkdir(exist_ok=True)
    (root / "architecture/manifest.yaml").write_text(
        json.dumps({"modules": [{"id": "fixture-device"}]})
    )
    allocate_run(
        root,
        "validation",
        output.name,
        {
            "target": {"module": "fixture-device"},
            "scenario": args[0],
            "tool": {"name": "unit-fixture", "version": "1"},
            "command": ["unittest"],
            "source": {"revision": "fixture", "dirty_sha256": "0" * 64},
            "inputs": {"effective_profile": profile_sha256(value)},
        },
    )
    return write_allocated_bundle(output, value, *args, **kwargs)
...
class EvidenceTests(unittest.TestCase):
    ...
    def test_bundle_records_user_upload_limitation(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            raw = root / "input.log"
            raw.write_text(good_log().replace("R1", "evidence"), encoding="utf-8")
            result = write_bundle(
                root / "artifacts/validation/evidence",
                profile(),
                "boot",
                raw,
                probe(profile()),
                expected_run_id="evidence",
            )
            self.assertEqual("PASS", result["verdict"])
            self.assertFalse(result["upload_verified_by_gpt"])
            self.assertTrue(
                (root / "artifacts/validation/evidence" / "raw.sha256").exists()
            )
            self.assertTrue(
                (root / "artifacts/validation/evidence" / "capability.json").exists()
            )
            self.assertTrue(
                (
                    root / "artifacts/validation/evidence" / "permission-decisions.json"
                ).exists()
            )
    ...
    def test_acceptance_requires_matching_enablement_pass_bundle(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            raw = root / "input.log"
            raw.write_text(raw_text.replace("R1", "blocked"), encoding="utf-8")
            blocked = write_bundle(
                root / "artifacts/validation/blocked",
                value,
                "accept",
                raw,
                probe(value),
                expected_run_id="blocked",
            )
            ...
            raw.write_text(raw_text.replace("R1", "stale"), encoding="utf-8")
            blocked = write_bundle(
                root / "artifacts/validation/stale",
                value,
                "accept",
                raw,
                probe(value),
            ...
            raw.write_text(raw_text.replace("R1", "passed"), encoding="utf-8")
            passed = write_bundle(
                root / "artifacts/validation/passed",
                value,
                "accept",
                raw,
                probe(value),
                expected_run_id="passed",
    ...
    def test_oversized_import_is_blocked_without_copying_raw(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertFalse(
                (root / "artifacts/validation/evidence" / "raw.log").exists()
            )
