...
resumes only with explicit evidence, including exact fresh-task `開始執行`; invalid
resume combinations block; read-only
...
availability, caller-supplied unresolved-decision evidence, exact fresh-task
confirmed-Spec resume intent, and optional
...
canonical spec resolution, and resume evidence are assumed to have passed their own
structural validation. The router derives resume evidence only from the full-match
phrase `開始執行`, optionally followed by one canonical Spec path; it never infers
continuity from general prompt wording.
...
- Exact fresh-task `開始執行` has the same effect as
  `resume_confirmed_spec=true`; quoted, negated, or longer uses have no such effect.
...
|---|---|---|---|
| Exact authorization token plus optional explicit Spec path, with boolean adapter support | Exact and auditable | Invalid or ambiguous evidence blocks | Focused unit and CLI fixtures cover one, many, zero, explicit path, quoted, and negated states. |
...
| Infer continuity from general prompt text | Model- and vocabulary-dependent | False resume or repeated interviews | Rejected because equivalent prompts cannot guarantee identical classification. |
...
`false`. Treat exact full-match `開始執行`, optionally followed by one canonical Spec
path, as the deterministic fresh-task form of the same evidence. Treat a resolved
confirmed spec as durable context only. With explicit resume evidence, verify and
resume exactly one valid confirmed spec; otherwise apply
...
General prompt inference is rejected because it is not deterministic or independently
auditable. The exact authorization token is a versioned full-match contract, not a
semantic guess. Unconditional unique-spec resume is rejected because spec discovery
does not prove that a request continues the same change set.
...
4. Preserve an explicit skill for all other requests; otherwise full-match exact
   `開始執行` with an optional canonical Spec path before classifying remaining hard
   intents in versioned order. Quoted, negated, and longer forms do not match.
...
No new callable or CLI parameter is introduced by the turn-boundary handoff.
...
`--resume-confirmed-spec`. `confirmed-spec-resume` is added to the versioned
`IntentAssessment.intent` enum, and `開始執行` is added to the versioned intent rules.
...
It is forbidden to infer resume evidence from general wording, treat quoted or
negated `開始執行` as authorization, auto-select the newest spec, bypass
