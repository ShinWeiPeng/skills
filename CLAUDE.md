...
For maintainer development only, `scripts/link-skills.sh` can link repository Skills into local harness directories and `scripts/install-local.ps1` can install an assembled local artifact. Supported users enter through the repository-root Windows CMD or Linux shell launcher; those native launchers provision bounded dependencies and install from `marketplace-release` through the `governed-engineering` Marketplace. ChatGPT web installation is unsupported.
