...
The governed OpenAI Plugin uses `skills/engineering` and `skills/productivity` as its only editable Skill source. Keep only its manifest, versioning, tests, and release infrastructure under `plugins/governed-engineering-skills`; never commit a second `skills/` tree there. Build `dist/governed-engineering-skills` with `python scripts/assemble_plugin.py assemble --marketplace-publication`, then run `python scripts/validate_distribution.py`. The ignored artifact is the candidate used by manual maintainer testing and to generate the governed Git Marketplace tree; the release workflow alone publishes that tree to `marketplace-release`.
...
For maintainer development only, `scripts/link-skills.sh` can link repository Skills into local harness directories and `scripts/install-local.ps1` can install an assembled local artifact. Supported users enter through the repository-root Windows CMD launcher, Linux Freedesktop GUI launcher, or Linux shell launcher; the GUI launcher delegates to the shell launcher, and those native launchers provision bounded dependencies and install from `marketplace-release` through the `governed-engineering` Marketplace. ChatGPT web installation is unsupported.
