...
from vod.providers import (
    capture_serial,
    capture_tcp,
    hash_evidence_reference,
    probe,
    read_bounded_json,
    write_json,
...
def _paths(args: argparse.Namespace) -> tuple[dict, list[str]]:
    return load_profile(
        args.profile.resolve(), args.local.resolve() if args.local else None
    )
...
def _external_contract_refs(profile: dict) -> set[str]:
    ...
    for port in manifest.get("ports", []):
        ...
        if isinstance(implemented_by, list) and adapter_ids.intersection(
            implemented_by
        ):
...
def _run_cleanups(
    profile: dict, scenario: dict, approvals: set[str], enabled: bool
) -> list[dict]:
    if not enabled:
        return []
    results = []
    for name in reversed(scenario.get("cleanup_actions", [])):
        try:
            ...
            result = {
                "status": "BLOCKED",
                "reason": f"cleanup raised {type(exc).__name__}: {exc}",
            }
...
def _guided_output_paths(output: Path) -> tuple[Path, Path, Path]:
    if output.suffix.lower() == ".json":
        ...
        return (
            output,
            output.with_name(prefix + "response.template.json"),
            output.with_name(prefix + "user-guide.md"),
        )
    return (
        output / "session.json",
...
def _guided_evaluation_context(
    args: argparse.Namespace, profile: dict, scenario_id: str
    ...
    if not args.guided_session:
        if args.guided_review or args.guided_response:
            return (
                expected_run_id,
                None,
                ["guided review/responses require --guided-session"],
            )
    ...
    artifacts: dict[str, object] = {
        "session": args.guided_session,
        "responses": [],
        "review": args.guided_review,
    }
    if args.guided_review:
        try:
            responses = [
                read_bounded_json(path, max_bytes) for path in args.guided_response
            ]
            review = read_bounded_json(args.guided_review, max_bytes)
            root = Path(profile["_project_root"])
            problems = validate_review_bundle(
                session,
                responses,
                review,
                lambda path: hash_evidence_reference(root, path, max_bytes),
        ...
        artifacts["review_markdown"] = (
            review_markdown if review_markdown.exists() else None
        )
        artifact_paths = [
            args.guided_session,
            *args.guided_response,
            args.guided_review,
            *([review_markdown] if review_markdown.exists() else []),
        ]
        artifacts["expected_sha256"] = {
            str(path.resolve()): hash_evidence_reference(root, str(path), max_bytes)[
                "sha256"
            ]
    ...
    return (
        ...
        artifacts,
        [
            "guided session requires --guided-review and at least one --guided-response for user-confirmed evaluation"
        ],
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for name in (
        "validate-profile",
        "probe",
        "prepare-guided-session",
        "finalize-guided-session",
        "capture",
        "run-action",
        "evaluate",
        "run",
        ...
        if name == "prepare-guided-session":
            ...
            command.add_argument(
                "--capture-mode",
                choices=("gpt-guided", "offline-user"),
                default="gpt-guided",
            )
            ...
            command.add_argument(
                "--guided-response", type=Path, action="append", default=[]
            )
            command.add_argument("--expected-run-id")
            command.add_argument(
                "--upload-actor", choices=("user", "gpt"), default="user"
            )
            command.add_argument(
                "--prerequisite-result", type=Path, action="append", default=[]
            )
            if name == "run":
                command.add_argument("--approve-risk", action="append", default=[])
    ...
    if args.command == "summarize-gates":
        ...
        try:
            ...
            return 2
        expected_enablement = {
            item["id"] for item in profile["scenarios"] if item["phase"] == "enablement"
        }
        expected_smoke = {
            item["id"] for item in profile["scenarios"] if item["phase"] == "smoke"
        }
        expected_acceptance = {
            item["id"] for item in profile["scenarios"] if item["phase"] == "acceptance"
        }
    ...
    if args.command == "prepare-guided-session":
        scenario = next(
            (item for item in profile["scenarios"] if item["id"] == args.scenario), None
        )
        if scenario is None:
        ...
        try:
            ...
            _emit(
                {
                    "status": "BLOCKED",
                    "reason": f"guided artifact already exists: {exc.filename}",
                }
            )
            return 2
        result = {
    ...
    if args.command == "finalize-guided-session":
        ...
        try:
            ...
            previous = (
                read_bounded_json(args.previous_response, max_bytes)
                if args.previous_response
                else None
            )
            root = Path(profile["_project_root"])
            review = finalize_response(
                session,
                response,
                lambda path: hash_evidence_reference(root, path, max_bytes),
                previous,
        ...
        if session.get("profile_sha256") != profile_sha256(profile):
            review.setdefault("problems", []).append(
                "session profile snapshot does not match the current profile"
            )
        ...
        try:
            ...
            _emit(
                {
                    "status": "BLOCKED",
                    "reason": f"review artifact already exists: {exc.filename}",
                }
            )
            return 2
        _emit(review)
    ...
    if args.command == "capture":
        ...
        if transport == "serial":
            result = capture_serial(
                profile, scenario, args.output, set(args.approve_risk)
            )
            ...
            result = {
                "status": "BLOCKED",
                "reason": f"transport {transport!r} requires guided native/import collection",
            }
        _emit(result)
    ...
    if args.command == "run-action":
        ...
        return (
            ...
            else (1 if result["status"] == "FAIL" else 2)
        )
    if args.command == "run":
        scenario = next(
            (item for item in profile["scenarios"] if item["id"] == args.scenario), None
        ...
        for action_name in scenario.get("actions", []):
            try:
                ...
                action_result = {
                    "status": "BLOCKED",
                    "reason": f"action raised {type(exc).__name__}: {exc}",
                    "spawned": True,
                }
            ...
            if action_result["status"] != "PASS":
                if (
                    ...
                    cleanup_results = _run_cleanups(
                        profile, scenario, approvals, cleanup_needed
                    )
                    cleanup_needed = False
                    cleanup_failed = next(
                        (item for item in cleanup_results if item["status"] != "PASS"),
                        None,
                    )
                    if cleanup_failed is not None:
                        args.output.mkdir(parents=True, exist_ok=True)
                        write_json(
                            args.output / "actions.json",
                            {"actions": action_results, "cleanups": cleanup_results},
                        )
                        _emit(
                            {
                                "status": "BLOCKED",
                                "reason": "trace cleanup failed before fallback",
                    ...
                    provider["fallback_reason"] = action_result.get(
                        "reason", "native trace action was blocked"
                    )
        ...
        if raw_path is None:
            ...
            try:
                if transport == "serial":
                    capture_result = capture_serial(
                        profile, scenario, raw_path, set(args.approve_risk)
                    )
                elif transport in {"tcp-client", "tcp-server"}:
                    ...
                    capture_result = {
                        "status": "BLOCKED",
                        "reason": "this transport requires --raw-log from a guided/import collection",
                    }
            except BaseException:
                cleanup_results.extend(
                    _run_cleanups(profile, scenario, approvals, cleanup_needed)
                )
                args.output.mkdir(parents=True, exist_ok=True)
                write_json(
                    args.output / "actions.json",
                    {
                        ...
                        "capture": {
                            "status": "BLOCKED",
                            "reason": "capture raised or was cancelled",
                        },
                    },
                )
                raise
        ...
        cleanup_failed = next(
            (item for item in cleanup_results if item["status"] != "PASS"), None
        )
        if cleanup_failed is not None:
            args.output.mkdir(parents=True, exist_ok=True)
            write_json(
            ...
            _emit(
                {
                    "status": "BLOCKED",
                    "reason": "required cleanup did not complete",
                    "cleanups": cleanup_results,
                }
            )
            return 2
        expected_run_id, guided_artifacts, guided_problems = _guided_evaluation_context(
            args, profile, args.scenario
        )
        if guided_problems:
            _emit(
                {
                    "status": "BLOCKED",
                    "reason": "guided review validation failed",
                    "problems": guided_problems,
                }
            )
            return 2
        if args.upload_actor == "user" and not expected_run_id:
            _emit(
                {
                    "status": "BLOCKED",
                    "reason": "user-uploaded evidence requires --guided-session or --expected-run-id to prevent replay",
                }
            )
            return 2
        ...
        for path in args.prerequisite_result:
            prerequisite_results.append(
                read_bounded_json(
                    path, int(profile.get("transport", {}).get("max_bytes", 1048576))
                )
            )
        result = write_bundle(
            args.output,
        ...
        return (
            ...
            else (1 if result["verdict"] == "FAIL" else 2)
        )
    ...
    result = write_bundle(
        ...
        args.upload_actor,
        args.native_evidence,
        native_source_path=args.native_source_trace,
        expected_run_id=expected_run_id,
        guided_artifacts=guided_artifacts,
