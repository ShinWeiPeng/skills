...
def run_action(
    profile: dict[str, Any], name: str, approvals: set[str]
) -> dict[str, Any]:
    action = profile.get("actions", {}).get(name)
    if not isinstance(action, dict):
        return {
            "status": "BLOCKED",
            "reason": f"action {name!r} is not declared",
            "spawned": False,
        }
    ...
    if risk not in PASSIVE_RISKS and risk not in approvals:
        return {
            "status": "BLOCKED",
            "reason": f"approval for risk {risk!r} is required",
            "risk": risk,
            "spawned": False,
        }
    ...
    cwd = (
        (project_root / cwd_value).resolve()
        if not cwd_value.is_absolute()
        else cwd_value.resolve()
    )
    if not cwd.is_relative_to(project_root):
        return {
            "status": "BLOCKED",
            "reason": "action cwd escapes the project root",
            "cwd": str(cwd),
            "spawned": False,
        }
    ...
    if action.get("bounded_artifact"):
        ...
        if not artifact_path.is_relative_to(project_root):
            return {
                "status": "BLOCKED",
                "reason": "bounded_artifact escapes the project root",
                "spawned": False,
            }
    ...
    try:
        ...
        return {
            "status": "BLOCKED",
            "reason": f"action raised {type(exc).__name__}: {exc}; process-tree termination confirmed={termination_confirmed}",
            "argv": argv,
            "cwd": str(cwd),
    ...
    exceeded = (
        ...
        or termination_reason == "bounded artifact reached max_artifact_bytes"
    )
    timed_out = termination_reason == "action timed out after execution began"
    return {
        "status": "FAIL"
        if timed_out
        else ("BLOCKED" if exceeded else ("PASS" if return_code == 0 else "FAIL")),
