...
def _default_steps(
    profile: dict[str, Any], scenario: dict[str, Any], run_id: str
) -> list[dict[str, Any]]:
    platform = profile["target"]["platform"]
    preparation = (
        "Prepare the declared test build and start the target application."
        if platform in {"windows", "linux", "ios"}
        else f"Upload the declared test firmware for run {run_id}."
    )
    steps: list[dict[str, Any]] = [
        {
            "id": "prepare-target",
            "instruction": preparation,
            "expected_observation": "The declared test build is present and the target is ready.",
            "required": True,
        ...
        {
            "id": "start-capture",
            "instruction": "Start bounded raw evidence capture before reset or trigger.",
            "expected_observation": "Capture is active and bound to this run ID.",
            "required": True,
            "evidence_required": False,
            "observation_code": None,
        },
        {
            "id": "begin-session",
            "instruction": f"Begin validation run {run_id} for scenario {scenario['id']}.",
            "expected_observation": "VAL_SESSION_BEGIN contains the same run and scenario.",
            "required": True,
            "evidence_required": False,
            "observation_code": None,
        },
    ...
    for criterion in scenario.get("criteria", []):
        if criterion.get("type") == "observation":
            steps.append(
                {
                    "id": f"observe-{criterion['id']}",
                    "instruction": f"Perform and observe step {criterion['step']} for {criterion['code']}.",
                    "expected_observation": f"Record the physical outcome for observation code {criterion['code']}.",
                    "required": True,
                    "evidence_required": False,
    ...
    steps.append(
        {
            ...
            "instruction": "End the session as complete and preserve the unedited evidence files.",
            "expected_observation": "VAL_SESSION_END is complete and the returned evidence files are unchanged.",
            "required": True,
...
def build_response_template(
    session: dict[str, Any], capture_mode: str = "offline-user"
) -> dict[str, Any]:
    return {
        "schema_version": "1.0",
...
def render_user_guide(session: dict[str, Any]) -> str:
    lines = [
        f"# Guided test: {session['scenario']}",
        "",
        f"Run ID: `{session['run_id']}`",
        f"Phase: `{session['phase']}`",
        f"Evidence mode: `{session['evidence_mode']}`",
        f"Maximum duration: `{session['max_duration_ms']} ms`",
        "",
        "Follow each form step in order. Record facts only; do not assign PASS or FAIL.",
        "",
    ]
    for step in session["steps"]:
        ...
        lines.extend(
            [
                f"## {step['order']}. {step['id']} ({required})",
                "",
                step["instruction"],
                "",
                f"Expected observable fact: {step['expected_observation']}",
                "",
                "- State: pending / completed / blocked",
                "- Actual observation:",
                "- Evidence file path(s):",
                "- If blocked, reason and remediation:",
                "",
            ]
        )
    lines.extend(
        [
            "## Final confirmation",
            "",
            "After reviewing the generated summary table, set `confirmation.confirmed` to `true` in the response JSON. This confirms only the recorded facts; the runner owns PASS/FAIL/BLOCKED.",
            "",
        ]
    )
    return "\n".join(lines)
...
def validate_session(session: Any) -> list[str]:
    ...
    if (
        ...
        or type(session.get("max_duration_ms")) is not int
        or session.get("max_duration_ms", 0) <= 0
    ):
    ...
    for index, step in enumerate(steps, 1):
        ...
        if isinstance(step_id, str):
            ids.add(step_id)
        if (
            type(step.get("required")) is not bool
            or type(step.get("evidence_required")) is not bool
...
def finalize_response(
    ...
    if problems:
        return {
            ...
            "scenario": session.get("scenario") if isinstance(session, dict) else None,
            "profile_sha256": session.get("profile_sha256")
            ...
            "supersedes_sha256": response.get("supersedes_sha256")
            if isinstance(response, dict)
            else None,
    ...
    if not isinstance(response, dict):
        return {
            "review_status": "BLOCKED",
            "problems": problems + ["response must be an object"],
        }
    if set(response) != RESPONSE_KEYS:
        problems.append(
            "response fields do not match the schema; verdict fields are forbidden"
        )
    ...
    if response.get("schema_version") != "1.0" or response.get("capture_mode") not in {
        "gpt-guided",
        "offline-user",
    }:
    ...
    if type(revision) is int and revision == 1:
        if (
            response.get("supersedes_sha256") is not None
            or previous_response is not None
        ):
            problems.append("revision 1 cannot supersede another response")
    elif type(revision) is int:
        if (
            previous_response is None
            or previous_response.get("revision") != revision - 1
            or response.get("supersedes_sha256") != canonical_sha256(previous_response)
        ):
            problems.append("revision chain is missing or invalid")
    confirmation = response.get("confirmation")
    confirmed = (
        isinstance(confirmation, dict)
        and set(confirmation) == CONFIRMATION_KEYS
    ...
    if not isinstance(response_steps, list) or len(response_steps) != len(
        expected_steps
    ):
    ...
    for index, expected in enumerate(expected_steps):
        ...
        if state == "completed" and (
            not isinstance(item.get("actual_observation"), str)
            or not item.get("actual_observation").strip()
        ):
            step_problems.append("completed step requires an actual observation")
        if state == "completed" and (
            item.get("blocked_reason") or item.get("remediation")
        ):
            step_problems.append(
                "completed step cannot retain blocked reason or remediation"
            )
        if state == "blocked" and (
            not isinstance(item.get("blocked_reason"), str)
            or not item.get("blocked_reason").strip()
            or not isinstance(item.get("remediation"), str)
            or not item.get("remediation").strip()
        ):
        ...
        if not isinstance(evidence, list):
            ...
            for ref in evidence:
                if (
                    ...
                    or set(ref) != EVIDENCE_KEYS
                    or not isinstance(ref.get("path"), str)
                    or not ref["path"]
                ):
                    step_problems.append(
                        "evidence reference must contain only a non-empty path"
                    )
                    continue
                try:
        ...
        problems.extend(
            f"step {expected['id']}: {problem}" for problem in step_problems
        )
    ...
    return {
        ...
        "supersedes_sha256": response.get("supersedes_sha256"),
        "review_status": "READY" if not problems else "BLOCKED",
...
def render_review(review: dict[str, Any]) -> str:
    ...
    lines = [
        f"# Guided test review: {review.get('scenario', '')}",
        "",
        f"Run ID: `{review.get('run_id', '')}`",
        "",
        f"Review status: **{review.get('review_status', 'BLOCKED')}**",
        "",
        "| # | Step | State | Actual observation | Evidence | Complete |",
        "|---:|---|---|---|---:|---|",
    ]
    for step in review.get("steps", []):
        lines.append(
            f"| {step['order']} | {cell(step['step_id'])} | {cell(step['state'])} | {cell(step['actual_observation'])} | {len(step['evidence'])} | {'yes' if step['complete'] else 'no'} |"
        )
    lines.extend(
        [
            "",
            f"- [ {'x' if review.get('confirmation', {}).get('confirmed') else ' '} ] The user confirmed that the recorded facts are correct.",
            "",
            "This table is read-only evidence review. It is not a PASS/FAIL decision source.",
            "",
        ]
    )
    if review.get("problems"):
        lines.extend(
            [
                "## Blocking problems",
                "",
                *[f"- {cell(problem)}" for problem in review["problems"]],
                "",
            ]
        )
    return "\n".join(lines)
...
def validate_review_bundle(
    ...
    for index, response in enumerate(responses, 1):
        ...
        if any(
            response.get(key) != session.get(key)
            for key in ("run_id", "scenario", "profile_sha256")
        ):
            problems.append(
                f"response revision {index} identity does not match session"
            )
        ...
        if (
            ...
            != expected_step_ids
            or any(
                not isinstance(step, dict) or set(step) != RESPONSE_STEP_KEYS
                for step in raw_steps
            )
        ):
            problems.append(f"response revision {index} step schema/order is invalid")
        elif any(
            not isinstance(ref, dict) or set(ref) != EVIDENCE_KEYS
            for step in raw_steps
            for ref in (
                step.get("evidence") if isinstance(step.get("evidence"), list) else [{}]
            )
        ):
            ...
            or any(
                not isinstance(ref.get("path"), str) or not ref.get("path")
    ...
    if isinstance(responses[-1], dict):
        computed_review = finalize_response(
            session,
            responses[-1],
            evidence_hasher,
            responses[-2] if len(responses) > 1 else None,
        )
    ...
    if (
        not isinstance(supplied_review, dict)
        or supplied_review.get("review_status") != "READY"
        or supplied_review.get("confirmation", {}).get("confirmed") is not True
    ):
