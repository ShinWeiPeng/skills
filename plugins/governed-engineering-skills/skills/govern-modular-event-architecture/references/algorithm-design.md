...
Schema 2.0.2 retains the execution-efficiency requirement to link the owning `ALG-####` to the applicable Workload, Execution Profile, Data Access Profile, Microarchitecture Profile, and Flow IDs. Tier 2 requires a portable baseline, representative benchmark composition, neighboring parameter candidates, and full-Flow regression evidence.
