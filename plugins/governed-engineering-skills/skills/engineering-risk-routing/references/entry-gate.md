...
1. Inspect the task and invoke `$engineering-risk-routing`; preserve the
   `GuidedRouteDecision.spec_context`.
...
4. For R2/R3 work, do not mutate before `clarify-improvement-proposals` and `govern-modular-event-architecture` have passed. Existing projects without supported governance first require an as-is inventory and remediation of discovered violations; only exact non-AI-approved temporary deferrals enter a baseline.
...
7. Every repository-modifying change set must resolve one canonical specification
   and complete `spec-verified: PASS` before TDD or implementation. Ambiguous or
   invalid spec context is `BLOCKED`; never select the newest candidate.
