...
def _confirmed_spec_resume_matches(
    prompt: str,
    config: dict[str, Any],
) -> str | None:
    normalized = prompt.strip().replace("\\", "/")
    canonical_path = r"specs/SPEC-[0-9]{4}-[^/\s]+\.md"
    for phrase in config["confirmed_spec_resume_phrases"]:
        if re.fullmatch(
            rf"{re.escape(phrase)}(?:\s+{canonical_path})?",
            normalized,
            flags=re.IGNORECASE,
        ):
            return phrase
...
def classify_intent(
    ...
    *,
    ...
    resume_match = _confirmed_spec_resume_matches(prompt, config)
    if resume_match and explicit_skill is None:
        return {
            "intent": "confirmed-spec-resume",
            "explicit_skill": None,
            "matched_terms": [resume_match],
...
def select_workflow(
    ...
    *,
    ...
    effective_resume = resume_confirmed_spec or intent == "confirmed-spec-resume"
    ...
    if modifies and effective_resume and spec_state != "confirmed":
        if spec_state == "none":
            resume_reason = (
                "No valid confirmed canonical specification exists; create and "
                "confirm one before starting execution."
            )
        elif spec_state == "ambiguous":
            resume_reason = (
                "Multiple confirmed canonical specifications match; ask the user "
                "to select exactly one candidate path."
            )
        else:
            resume_reason = (
    ...
    if intent == "review":
        ...
        post_spec_target = (
            ...
            if intent == "confirmed-spec-resume"
            else "to-spec"
