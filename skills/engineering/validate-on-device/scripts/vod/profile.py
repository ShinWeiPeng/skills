...
def required_samples(plan: dict[str, Any]) -> int:
    ...
    if model == "distribution":
        return max(
            1, math.ceil(math.log(2.0 / (1.0 - confidence)) / (2.0 * error * error))
        )
    z = NormalDist().inv_cdf((1.0 + confidence) / 2.0)
    if model == "proportion":
        proportion = float(plan["expected_proportion"])
        return max(
            1, math.ceil(z * z * proportion * (1.0 - proportion) / (error * error))
        )
    if model == "mean":
...
def _validate_sample_plan(plan: Any, path: str) -> list[str]:
    ...
    if basis != "calculated":
        errors.append(
            f"{path}.sample_plan.basis must be calculated or external-standard"
        )
        return errors
    if plan.get("model") not in {"proportion", "mean", "distribution"}:
        errors.append(
            f"{path}.sample_plan.model must be proportion, mean, or distribution"
        )
    ...
    if plan.get("model") == "proportion" and (
        not _number(plan.get("expected_proportion"))
        or plan.get("expected_proportion", 2) > 1
    ):
        errors.append(
            f"{path}.sample_plan.expected_proportion must be between zero and one"
        )
    if plan.get("model") == "mean" and not _number(
        plan.get("estimated_stddev"), positive=True
    ):
        errors.append(f"{path}.sample_plan.estimated_stddev must be positive")
    return errors


def validate_local_override(
    local: dict[str, Any], tracked: dict[str, Any]
) -> list[str]:
    errors: list[str] = []
    for key in local:
        if key not in {"bindings", "actions"}:
            errors.append(
                f"local override may not change {key!r}; only bindings and action executable paths are allowed"
            )
    ...
    if bindings and not isinstance(bindings, dict):
        ...
        for name, binding in bindings.items():
            if not isinstance(binding, dict) or set(binding) - allowed_binding_keys:
                errors.append(
                    f"local bindings.{name} may contain only endpoint identity fields {sorted(allowed_binding_keys)}"
                )
    if actions and not isinstance(actions, dict):
        ...
        for name, override in actions.items():
            if name not in tracked.get("actions", {}):
                errors.append(
                    f"local actions.{name} is not declared in the tracked profile"
                )
...
def _find_literal_secrets(value: Any, path: str = "") -> list[str]:
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            if any(word in str(key).lower() for word in SECRET_WORDS) and child not in (
                None,
                "",
            ):
...
def validate_profile(profile: dict[str, Any], *, tracked: bool = True) -> list[str]:
    ...
    if architecture is not None:
        if not isinstance(architecture, dict):
            ...
            if not isinstance(
                architecture.get("manifest"), str
            ) or not architecture.get("manifest"):
                errors.append("architecture.manifest is required")
            if version == "1.1":
                if not isinstance(
                    architecture.get("manifest_sha256"), str
                ) or not re.fullmatch(
                    r"[0-9a-f]{64}", architecture.get("manifest_sha256", "")
                ):
                    errors.append(
                        "architecture.manifest_sha256 must be 64 lowercase hex characters"
                    )
                if not isinstance(
                    architecture.get("execution_profile"), str
                ) or not architecture.get("execution_profile"):
                    errors.append(
                        "architecture.execution_profile is required for version 1.1"
                    )
    ...
    if (
        not isinstance(target.get("native_max_bytes", 4294967296), int)
        or target.get("native_max_bytes", 4294967296) <= 0
    ):
    ...
    if transport and transport.get("type") not in {
        "serial",
        "tcp-client",
        "tcp-server",
        "native",
    ...
    for name, action in actions.items():
        ...
        if not isinstance(action.get("executable"), str) or not action.get(
            "executable"
        ):
            errors.append(f"actions.{name}.executable is required")
        if not isinstance(action.get("args", []), list) or not all(
            isinstance(x, str) for x in action.get("args", [])
        ):
        ...
        if action.get("risk", "passive") == "trace":
            ...
            if action.get("trace_role") == "record" and not action.get(
                "bounded_artifact"
            ):
        ...
        if (
            not isinstance(action.get("max_output_bytes", 1048576), int)
            or action.get("max_output_bytes", 1048576) <= 0
        ):
            errors.append(f"actions.{name}.max_output_bytes must be a positive integer")
        if action.get("bounded_artifact") is not None and (
            not isinstance(action.get("bounded_artifact"), str)
            or not action.get("bounded_artifact")
            or not isinstance(action.get("max_artifact_bytes"), int)
            or action.get("max_artifact_bytes", 0) <= 0
        ):
            errors.append(
                f"actions.{name}.bounded_artifact requires a path and positive max_artifact_bytes"
            )
    ...
    for index, scenario in enumerate(scenarios):
        ...
        for key in ("title", "purpose"):
            if key in scenario and (
                not isinstance(scenario[key], str) or not scenario[key].strip()
            ):
        ...
        for key in ("trigger_criteria", "required_criteria"):
            if (
                not isinstance(completion.get(key), list)
                or not completion.get(key)
                or not all(
                    isinstance(item, str) and item for item in completion.get(key, [])
                )
            ):
                errors.append(
                    f"{prefix}.completion.{key} must be a non-empty criterion ID list"
                )
        if not isinstance(
            completion.get("session_end_reason"), str
        ) or not completion.get("session_end_reason"):
        ...
        if not isinstance(guided_steps, list):
            ...
            for step_index, step in enumerate(guided_steps):
                ...
                if not isinstance(step, dict) or set(step) - GUIDED_STEP_KEYS:
                    errors.append(
                        f"{spath} contains unknown fields or is not a mapping"
                    )
                    continue
                step_id = step.get("id")
        ...
        for key in ("trigger_criteria", "required_criteria"):
            for criterion_id in (
                completion.get(key, []) if isinstance(completion.get(key), list) else []
            ):
                if criterion_id not in criterion_ids:
                    errors.append(
                        f"{prefix}.completion.{key} references unknown criterion {criterion_id!r}"
                    )
        flow_ids = {
            c.get("id")
            for c in criteria
            if isinstance(c, dict)
            and c.get("type") in {"event_sequence", "observation"}
        }
        statistical_ids = {
            c.get("id")
            for c in criteria
            if isinstance(c, dict) and c.get("type") in {"statistic", "native_metric"}
        }
        required_ids = set(completion.get("required_criteria", []))
        if evidence_mode in {"flow", "mixed"} and not required_ids.intersection(
            flow_ids
        ):
            errors.append(f"{prefix}.completion requires at least one flow criterion")
        if evidence_mode in {"statistical", "mixed"} and not required_ids.intersection(
            statistical_ids
        ):
            errors.append(
                f"{prefix}.completion requires at least one statistical criterion"
            )
        scenario_actions = scenario.get("actions", [])
        cleanup_actions = scenario.get("cleanup_actions", [])
        if not isinstance(scenario_actions, list) or not all(
            isinstance(name, str) and name in actions for name in scenario_actions
        ):
            errors.append(f"{prefix}.actions must reference declared actions")
            scenario_actions = []
        if not isinstance(cleanup_actions, list) or not all(
            isinstance(name, str) and name in actions for name in cleanup_actions
        ):
        ...
        for name in cleanup_actions:
            ...
            if (
                cleanup.get("idempotent") is not True
                or cleanup.get("risk", "passive") not in {"passive", "trace"}
                or (
                    cleanup.get("risk") == "trace"
                    and cleanup.get("trace_role") not in {"stop", "cleanup"}
                )
            ):
                errors.append(
                    f"actions.{name} cleanup must declare idempotent: true and use passive or trace risk"
                )
        if (
            any(actions[name].get("risk") == "trace" for name in scenario_actions)
        ...
        if scenario.get("warmup_required", False):
            statistical_metrics = {
                c.get("metric")
                for c in criteria
                if isinstance(c, dict) and c.get("type") == "statistic"
            }
            hard_limit_metrics = {
                c.get("metric")
                for c in criteria
                if isinstance(c, dict) and c.get("type") == "hard_limit"
            }
            for reused in sorted((statistical_metrics & hard_limit_metrics) - {None}):
                errors.append(
                    f"{prefix}: warmup statistic and hard_limit must use distinct metrics, not {reused!r}"
                )
        for c_index, criterion in enumerate(criteria):
            ...
            for key in ("label", "description"):
                if key in criterion and (
                    not isinstance(criterion[key], str) or not criterion[key].strip()
                ):
            ...
            if criterion.get("type") == "event_sequence" and not criterion.get(
                "events"
            ):
                errors.append(f"{cpath}.events is required")
            if criterion.get("type") == "observation":
                if not isinstance(criterion.get("code"), str) or not criterion.get(
                    "code"
                ):
                    errors.append(f"{cpath}.code is required")
                if not isinstance(criterion.get("step"), int) or isinstance(
                    criterion.get("step"), bool
                ):
                    errors.append(f"{cpath}.step must be an integer")
            if criterion.get("type") in {
                "hard_limit",
                "statistic",
                "native_metric",
            } and not criterion.get("metric"):
            ...
            if criterion.get("type") in {"hard_limit", "statistic", "native_metric"}:
                if criterion.get("operator", "<=") not in OPERATORS:
                    errors.append(
                        f"{cpath}.operator must be one of {sorted(OPERATORS)}"
                    )
            ...
            if criterion.get("type") == "statistic":
                if criterion.get("method", "field") not in STATISTIC_METHODS:
                    errors.append(
                        f"{cpath}.method must be one of {sorted(STATISTIC_METHODS)}"
                    )
                if criterion.get("method") == "percentile" and (
                    not _number(criterion.get("percentile"), positive=True)
                    or criterion.get("percentile", 0) > 1
                ):
                    errors.append(f"{cpath}.percentile must be between zero and one")
                errors.extend(
                    _validate_sample_plan(criterion.get("sample_plan"), cpath)
                )
                ...
                if criterion.get("timing", False) and not isinstance(
                    criterion.get("instrumentation_budget"), dict
                ):
                    errors.append(
                        f"{cpath}.instrumentation_budget is required for timing statistics"
                    )
                if criterion.get("timing", False):
                    ...
                    for key in (
                        ...
                        if not _number(budget.get(key)):
                            errors.append(
                                f"{cpath}.instrumentation_budget.{key} must be a non-negative number"
                            )
                    clock = criterion.get("clock")
                    if (
                        not isinstance(clock, dict)
                        or not isinstance(clock.get("source"), str)
                        or not clock.get("source")
                    ):
                        errors.append(
                            f"{cpath}.clock.source is required for timing statistics"
                        )
                    else:
                        ...
                        if not _number(clock.get("resolution_ns"), positive=True):
                            errors.append(
                                f"{cpath}.clock.resolution_ns must be positive"
                            )
            if criterion.get("type") == "native_metric":
                if not _number(criterion.get("max_alignment_error_ns")):
                    errors.append(
                        f"{cpath}.max_alignment_error_ns must be a non-negative number"
                    )
                errors.extend(
                    _validate_sample_plan(criterion.get("sample_plan"), cpath)
                )
                ...
                if not isinstance(semantics, dict) or not all(
                    semantics.get(key) for key in ("start_event", "end_event", "clock")
                ):
                    errors.append(
                        f"{cpath}.native_semantics must define start_event, end_event, and clock"
                    )
                if not isinstance(
                    criterion.get("native_unit"), str
                ) or not criterion.get("native_unit"):
    ...
    required_native = {
        "windows": "etw-wpr",
        "linux": "perf-ftrace",
        "ios": "instruments-xctest",
    }
    platform = target.get("platform")
    if (
        has_native_resource
        and platform in required_native
        and target.get("provider") != required_native[platform]
    ):
        errors.append(
            f"target.provider must be {required_native[platform]!r} for {platform} native resource criteria"
        )
    for index, scenario in enumerate(scenarios):
        if not isinstance(scenario, dict):
            continue
        for prerequisite in scenario.get("prerequisites", []):
            ...
            if target_scenario is None:
                errors.append(
                    f"scenarios[{index}].prerequisites references unknown scenario {prerequisite!r}"
                )
            elif target_scenario.get("phase") != "enablement":
                errors.append(
                    f"scenarios[{index}].prerequisites must reference enablement scenarios"
                )
        if scenario.get("phase") == "acceptance" and not scenario.get("prerequisites"):
            errors.append(
                f"scenarios[{index}].prerequisites requires at least one enablement scenario for acceptance"
            )
    errors.extend(_find_literal_secrets(profile))
    return errors


def validate_architecture_references(
    ...
    if str(profile.get("version")) == "1.1":
        ...
        if architecture.get("manifest_sha256") != actual_hash:
            errors.append(
                "architecture.manifest_sha256 does not match the selected manifest"
            )
        ...
        if selected is None:
            errors.append(
                f"architecture.execution_profile references unknown profile {profile_id!r}"
            )
        elif selected.get("status") != "accepted":
            errors.append(
                "architecture.execution_profile must reference an accepted profile"
            )
        else:
            execution_platform = selected.get("target", {}).get(
                "validation_platform", selected.get("target", {}).get("platform")
            )
    ...
    for scenario in profile.get("scenarios", []):
        refs = (
            scenario.get("architecture_refs", []) if isinstance(scenario, dict) else []
        )
        if not refs:
            errors.append(
                f"scenario {scenario.get('id', '<unknown>')}: architecture_refs is required for a governed project"
            )
        for ref in refs:
            if ref not in known:
                errors.append(
                    f"scenario {scenario.get('id', '<unknown>')}: unknown architecture reference {ref!r}"
                )
    return errors


def load_profile(
    profile_path: Path, local_path: Path | None = None
...
def redact(value: Any, key: str = "") -> Any:
    ...
    if isinstance(value, dict):
        return {
            item_key: redact(item, str(item_key)) for item_key, item in value.items()
        }
