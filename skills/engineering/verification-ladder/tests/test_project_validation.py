...
def save_plan(root):
    ...
    write(root, "spec-governance/validation-SPEC-0001.json", assessment["plan"])
    return assessment
...
def evidence(root, planned):
    ...
    for layer, scenario, purpose in [
        ...
        runner_path = f"evidence/{scenario}.json"
        write(
            root,
            runner_path,
            {
    ...
    write(root, "validation/evidence-SPEC-0001.json", result)
    return result
...
class ProjectValidationTests(unittest.TestCase):
    ...
    def test_planning_allows_work_but_acceptance_requires_hil(self):
        with tempfile.TemporaryDirectory(dir=TEST_TEMP_ROOT) as directory:
            ...
            write(root, "validation/evidence-SPEC-0001.json", enabled)
    ...
    def test_host_build_smoke_partial_wrong_profile_and_stale_artifacts_do_not_pass(
        ...
        for case in cases:
            with (
                ...
                if case == "artifact":
                    write(root, "evidence/hil-timing.json", "changed")
                if case == "incomplete":
                    row["complete"] = False
                if case == "duplicate":
                ...
                write(root, "validation/evidence-SPEC-0001.json", bundle)
                self.assertEqual(
                    validation.assess_project(
                        root, "specs/SPEC-0001-sensor.md", phase="acceptance"
                    )["verdict"],
    ...
    def test_lifecycle_and_managed_completion_reject_host_only_pass(self):
        with tempfile.TemporaryDirectory(dir=TEST_TEMP_ROOT) as directory:
            ...
            write(root, "validation/evidence-SPEC-0001.json", full_bundle)
            self.assertEqual(
                execute_request(
                    root,
                    base | {"operation": "complete"},
    ...
    def test_release_wraps_real_acceptance_runner_instead_of_nonexistent_release_phase(
        ...
        with tempfile.TemporaryDirectory(dir=TEST_TEMP_ROOT) as directory:
            ...
            row["purpose"] = "release"
            row["release_report"] = "evidence/release.json"
            report = {
