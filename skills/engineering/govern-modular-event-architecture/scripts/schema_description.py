...
def _symbol_entries(
    ...
    if not entries:
        _diag(
            ...
            "SYM002",
            f"{module_id}.{field}",
            "must declare at least one code symbol",
        )
        ...
        for key in ("path", "symbol", "kind"):
            if not _is_nonempty_string(entry.get(key)):
                ...
                _diag(
                    ...
                    "SYM003",
                    f"{location}.{key}",
                    "implemented module cannot contain a TODO symbol placeholder",
                )
...
def _check_declared_path(
    ...
    if not (project_root / relative).exists():
        ...
        _diag(
            ...
            rule,
            location,
            f"declared path does not exist: {declared}",
            severity=severity,
        )
    ...
    *,
    ...
    if data.get("standard_version") != DESCRIPTION_STANDARD_VERSION:
        _diag(
            ...
            "VER001",
            "standard_version",
            f"must equal {DESCRIPTION_STANDARD_VERSION!r}",
            configuration=True,
        )
    if data.get("schema_version") != DESCRIPTION_SCHEMA_VERSION:
        _diag(
            ...
            "VER001",
            "schema_version",
            f"must equal {DESCRIPTION_SCHEMA_VERSION!r}",
            configuration=True,
        )
    ...
    if not _is_nonempty_string(project.get("documentation_language")):
        _diag(
            diagnostics,
            "DESC001",
            "project.documentation_language",
            "must be a non-empty language tag",
            configuration=True,
        )
    ...
    for raw in projected.get("modules", []):
        if isinstance(raw, dict):
            ...
            raw["responsibility"] = description.get(
                "purpose", "missing description purpose"
            )
    ...
    modules = {
        ...
        if isinstance(raw, dict) and _is_nonempty_string(raw.get("id"))
    }
    ports = {
        str(raw.get("id")): raw
    ...
    for module_id, module in modules.items():
        ...
        if "responsibility" in module:
            _diag(
                ...
                "DESC003",
                module_id,
                "schema 1.1 replaces responsibility with description.purpose",
            )
        ...
        if status not in {"planned", "implemented"}:
            _diag(
                diagnostics,
                "DESC004",
                f"{module_id}.implementation_status",
        ...
        if not _is_nonempty_string(purpose):
            _diag(
                diagnostics,
                "DESC006",
                f"{module_id}.description.purpose",
                "must be non-empty",
                configuration=True,
            )
            ...
            _diag(
                ...
                "DESC007",
                f"{module_id}.description.purpose",
                "implemented module cannot contain TODO text",
            )

        input_ports = _string_list(
            description.get("input_ports"),
            diagnostics,
            "DESC008",
            f"{module_id}.description.input_ports",
        )
        output_ports = _string_list(
            description.get("output_ports"),
            diagnostics,
            "DESC008",
            f"{module_id}.description.output_ports",
        )
        emitted_events = _string_list(
            description.get("emitted_events"),
            diagnostics,
            "DESC008",
            f"{module_id}.description.emitted_events",
        )
        for port_id in input_ports:
            if (
                port_id not in ports
                or ports[port_id].get("owner") != module_id
                or ports[port_id].get("direction") != "input"
            ):
                _diag(
                    diagnostics,
                    "REF001",
                    f"{module_id}.description.input_ports",
                    f"invalid owned input port {port_id!r}",
                )
        for port_id in output_ports:
            if (
                port_id not in ports
                or ports[port_id].get("owner") != module_id
                or ports[port_id].get("direction") != "output"
            ):
                _diag(
                    diagnostics,
                    "REF002",
                    f"{module_id}.description.output_ports",
                    f"invalid owned output port {port_id!r}",
                )
        for event_id in emitted_events:
            if event_id not in events or events[event_id].get("owner") != module_id:
                _diag(
                    ...
                    "REF003",
                    f"{module_id}.description.emitted_events",
                    f"invalid owned event {event_id!r}",
                )

        states = _list(
            description.get("owned_state"),
            diagnostics,
            "DESC009",
            f"{module_id}.description.owned_state",
        )
        for index, raw_state in enumerate(states):
            state = _mapping(
                ...
                diagnostics,
                "DESC009",
                f"{module_id}.description.owned_state[{index}]",
            )
            if not _is_nonempty_string(state.get("name")) or not _is_nonempty_string(
        ...
        effects = _list(
            description.get("side_effects"),
            diagnostics,
            "DESC010",
            f"{module_id}.description.side_effects",
        )
        for index, raw_effect in enumerate(effects):
            effect = _mapping(
                ...
                diagnostics,
                "DESC010",
                f"{module_id}.description.side_effects[{index}]",
            )
            if not _is_nonempty_string(effect.get("description")):
                _diag(
                    diagnostics,
                    "DESC010",
                    f"{module_id}.description.side_effects[{index}].description",
                    "must be non-empty",
                    configuration=True,
                )
            ...
            if via_port is not None and via_port not in ports:
                _diag(
                    diagnostics,
                    "REF004",
                    f"{module_id}.description.side_effects[{index}].via_port",
                    f"unknown port {via_port!r}",
                )
        ...
        errors = _list(
            ...
            diagnostics,
            "DESC011",
            f"{module_id}.description.errors",
        )
        for index, raw_error in enumerate(errors):
            error = _mapping(
                ...
                diagnostics,
                "DESC011",
                f"{module_id}.description.errors[{index}]",
            )
            for key in ("id", "condition", "event", "handling"):
                if not _is_nonempty_string(error.get(key)):
                    _diag(
                        diagnostics,
                        "DESC011",
                        f"{module_id}.description.errors[{index}].{key}",
                        "must be non-empty",
                        configuration=True,
                    )
            if _is_nonempty_string(error.get("event")) and error["event"] not in events:
                _diag(
                    diagnostics,
                    "REF005",
                    f"{module_id}.description.errors[{index}].event",
                    f"unknown event {error['event']!r}",
                )
        _string_list(
            description.get("invariants"),
            diagnostics,
            "DESC012",
            f"{module_id}.description.invariants",
        )

        entrypoints = _symbol_entries(
            module_id,
            "entrypoints",
        ...
        public_symbols = _symbol_entries(
            module_id,
            "public_symbols",
        ...
        suffixes = {
            Path(str(entry.get("path", ""))).suffix.lower()
            for entry in entrypoints + public_symbols
        }
        if suffixes and not suffixes.issubset(C_SUFFIXES):
            _diag(
                ...
                "SYM004",
                module_id,
                "one or more symbols have no installed language analyzer",
                severity="SHOULD",
            )

    for port_id, port in ports.items():
        description = _mapping(
            port.get("description"), diagnostics, "PRD001", f"{port_id}.description"
        )
        owner_status = modules.get(str(port.get("owner")), {}).get(
        ...
        for key in ("purpose", "data"):
            if not _is_nonempty_string(description.get(key)):
                ...
                _diag(
                    ...
                    "PRD006",
                    f"{port_id}.description.{key}",
                    "implemented port cannot contain TODO text",
                )
        if description.get("timing") not in {"sync", "async"}:
            _diag(
                diagnostics,
                "PRD003",
                f"{port_id}.description.timing",
                "must be sync or async",
                configuration=True,
            )
        rejections = _list(
            description.get("immediate_rejections"),
            diagnostics,
            "PRD004",
            f"{port_id}.description.immediate_rejections",
        )
        for index, raw_rejection in enumerate(rejections):
            rejection = _mapping(
                ...
                diagnostics,
                "PRD004",
                f"{port_id}.description.immediate_rejections[{index}]",
            )
            if not _is_nonempty_string(
                rejection.get("code")
            ) or not _is_nonempty_string(rejection.get("condition")):
                _diag(
                    diagnostics,
                    "PRD004",
                    f"{port_id}.description.immediate_rejections[{index}]",
                    "requires code and condition",
                    configuration=True,
                )
        symbols = _string_list(
            port.get("symbols"), diagnostics, "PRD005", f"{port_id}.symbols"
        )
        owner = modules.get(str(port.get("owner")), {})
        owner_symbols = {
            str(item.get("symbol"))
            for item in owner.get("public_symbols", [])
        ...
        for symbol in symbols:
            if symbol not in owner_symbols:
                _diag(
                    ...
                    "REF006",
                    f"{port_id}.symbols",
                    f"symbol {symbol!r} is not declared by the owner module",
                )

    for event_id, event in events.items():
        description = _mapping(
            event.get("description"), diagnostics, "EVD001", f"{event_id}.description"
        )
        owner_status = modules.get(str(event.get("owner")), {}).get(
            "implementation_status"
        )
        for key in ("purpose", "emitted_when"):
            if not _is_nonempty_string(description.get(key)):
                ...
                _diag(
                    ...
                    "EVD005",
                    f"{event_id}.description.{key}",
                    "implemented event cannot contain TODO text",
                )
        fields = _list(
            description.get("payload_fields"),
            diagnostics,
            "EVD003",
            f"{event_id}.description.payload_fields",
        )
        for index, raw_field in enumerate(fields):
            field = _mapping(
                ...
                diagnostics,
                "EVD003",
                f"{event_id}.description.payload_fields[{index}]",
            )
            if any(
                not _is_nonempty_string(field.get(key))
                ...
                _diag(
                    ...
                    "EVD003",
                    f"{event_id}.description.payload_fields[{index}]",
                    "requires name, type, and meaning",
                    configuration=True,
                )
        consumers = _string_list(
            description.get("intended_consumers"),
            diagnostics,
            "EVD004",
            f"{event_id}.description.intended_consumers",
        )
    ...
    for index, raw_flow in enumerate(raw_flows):
        ...
        if owner is None or owner.get("level") not in {"L0", "L1"}:
            _diag(
                ...
                "FLW003",
                str(flow_id),
                "flow owner must be an L0 or L1 module",
            )
        ...
        trigger = _mapping(
            flow.get("trigger"), diagnostics, "FLW005", f"{flow_id}.trigger"
        )
        ...
        if kind not in {"command", "event", "condition"}:
            _diag(
                diagnostics,
                "FLW005",
                f"{flow_id}.trigger.kind",
                "must be command, event, or condition",
                configuration=True,
            )
        if not _is_nonempty_string(ref) or not _is_nonempty_string(
            ...
            _diag(
                diagnostics,
                "REF008",
                f"{flow_id}.trigger.ref",
                f"unknown command port {ref!r}",
            )
        ...
        for step_index, raw_step in enumerate(steps):
            step = _mapping(
                raw_step, diagnostics, "FLW006", f"{flow_id}.steps[{step_index}]"
            )
            ...
            if not isinstance(order, int):
                _diag(
                    diagnostics,
                    "FLW007",
                    f"{flow_id}.steps[{step_index}].order",
                    "must be an integer",
                    configuration=True,
                )
            else:
            ...
            if step.get("module") not in modules:
                _diag(
                    diagnostics,
                    "REF009",
                    f"{flow_id}.steps[{step_index}].module",
                    f"unknown module {step.get('module')!r}",
                )
            if not _is_nonempty_string(step.get("action")):
                _diag(
                    diagnostics,
                    "FLW008",
                    f"{flow_id}.steps[{step_index}].action",
                    "must be non-empty",
                    configuration=True,
                )
            for key in ("receives", "emits"):
                refs = _string_list(
                    ...
                    diagnostics,
                    "FLW009",
                    f"{flow_id}.steps[{step_index}].{key}",
                )
                for item in refs:
                    if item not in ports and item not in events:
                        _diag(
                            ...
                            "REF010",
                            f"{flow_id}.steps[{step_index}].{key}",
                            f"unknown port or event {item!r}",
                        )
                    referenced.add(item)
            _string_list(
                step.get("state_changes"),
                diagnostics,
                "FLW010",
                f"{flow_id}.steps[{step_index}].state_changes",
            )
            _string_list(
                step.get("side_effects"),
                diagnostics,
                "FLW010",
                f"{flow_id}.steps[{step_index}].side_effects",
            )
        if orders != list(range(1, len(steps) + 1)):
            _diag(
                ...
                "FLW011",
                f"{flow_id}.steps",
                "step order must be unique and contiguous starting at 1",
            )

        success = _mapping(
            flow.get("success"), diagnostics, "FLW012", f"{flow_id}.success"
        )
        if not _is_nonempty_string(success.get("result")):
            _diag(
                diagnostics,
                "FLW012",
                f"{flow_id}.success.result",
                "must be non-empty",
                configuration=True,
            )
        for event_ref in _string_list(
            success.get("events"), diagnostics, "FLW012", f"{flow_id}.success.events"
        ):
            if event_ref not in events:
                _diag(
                    diagnostics,
                    "REF011",
                    f"{flow_id}.success.events",
                    f"unknown event {event_ref!r}",
                )
            referenced.add(event_ref)
        for error_index, raw_error in enumerate(
            _list(flow.get("errors"), diagnostics, "FLW013", f"{flow_id}.errors")
        ):
            error = _mapping(
                raw_error, diagnostics, "FLW013", f"{flow_id}.errors[{error_index}]"
            )
            for key in ("condition", "event", "handling"):
                if not _is_nonempty_string(error.get(key)):
                    _diag(
                        diagnostics,
                        "FLW013",
                        f"{flow_id}.errors[{error_index}].{key}",
                        "must be non-empty",
                        configuration=True,
                    )
            if _is_nonempty_string(error.get("event")) and error["event"] not in events:
                _diag(
                    diagnostics,
                    "REF012",
                    f"{flow_id}.errors[{error_index}].event",
                    f"unknown event {error['event']!r}",
                )
    ...
    for port_id, port in ports.items():
        if (
            port.get("kind") == "command"
            and port.get("direction") == "input"
            and port_id not in referenced
        ):
            _diag(
                diagnostics,
                "FLW014",
                port_id,
                "public command is not referenced by a flow",
                severity="SHOULD",
            )
    for event_id in events:
        if event_id not in referenced:
            _diag(
                ...
                "FLW015",
                event_id,
                "event is not referenced by a flow",
                severity="SHOULD",
            )

    if check_docs and not any(
        item.configuration and item.disposition == "active" for item in diagnostics
    ):
