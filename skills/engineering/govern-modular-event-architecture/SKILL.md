...
4. For an architecture proposal, invoke `$clarify-improvement-proposals`. Follow the current mode's mutation boundary, and follow the shared Decision Question Contract: numbered text in Default/execution mode, the available choice surface when already in Plan mode. Do not provide a preliminary architecture unless the user explicitly requests one.
